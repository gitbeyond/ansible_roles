...
# centos6 test
```bash
[root@securetest yum.repos.d]# passwd test1
Changing password for user test1.
New password: abc123
...
faillock 命令可以看到相关信息。
```bash
[root@securetest yum.repos.d]# faillock 
test1:
...
```bash
[root@securetest yum.repos.d]# faillock --user test1 --reset
[root@securetest yum.repos.d]# faillock 
test1:
When                Type  Source                                           Valid
```
