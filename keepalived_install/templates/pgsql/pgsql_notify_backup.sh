...
write_log_failover "backup notify. keepalived VIP switchover!"
...
sql_execute(){
    local sql_words=$1
    # sudo -i -u postgres ${PGHOME}/bin/psql -At -p ${PGPORT} -U ${PGUSER} -d ${PGDBNAME} -c "${sql_words}"
    sudo -i -u postgres ${PGHOME}/bin/psql -At -U ${PGUSER} -d ${PGDBNAME} -c "${sql_words}"
}
